...
def check_var_problem(a, L, w):
    ...
    assert np.allclose(dat1[0], dat2[0], rtol=1.e-5), "Velocity in solution does not solve the variational problem."
    assert np.allclose(dat1[1], dat2[1], rtol=1.e-5), "Pressure in solution does not solve the variational problem."
...
def project_trace_solution(T, exact_sol, degree):
    ...
    solver_t = LinearVariationalSolver(vpb_t, solver_parameters={'ksp_rtol': 1.e-5})
    solver_t.solve()
    return w_t_exact