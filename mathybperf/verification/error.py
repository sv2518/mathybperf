...
def check_error(w, w2):
    ...
    assert np.allclose(sigma.dat.data, sigma2.dat.data, rtol=1.e-4), norm
    norm = errornorm(u, u2, norm_type="L2")
    assert np.allclose(u.dat.data, u2.dat.data, rtol=1.e-4), norm
...
def check_var_problem(a, L, w):
    ...
    assert np.allclose(dat1[0], dat2[0], rtol=1.e-4), "Velocity in solution does not solve the variational problem."
    assert np.allclose(dat1[1], dat2[1], rtol=1.e-4), "Pressure in solution does not solve the variational problem."
